...
async def start_game(гравець1: str, гравець2: str, номер_гравця_1: str, номер_гравця_2: str):
    ...
    return {"message": "Game starts ", "Player1": гравець1, "Player2": гравець2}
...
async def move(player: str, start: str, end: str):
    ...
    if isinstance(piece, str) and "ПІШАК" in piece.lower():
    ...
    if piece_instance is None:
        raise HTTPException(status_code=400, detail="this move is illegal")
...
@app.get("/Board", response_class=HTMLResponse)  # тільки для візуалізації і не більше
async def get_piece_list():
    if game is None or game.board is None or game.board.board is None:
        return HTMLResponse(content="Game is not working.")
...
async def get_match_results():
    ...
    table_content = "<h2>Results</h2><table border='1'><tr><th>ID</th><th>First Player</th><th>Second Player</th><th>Winner</th><th>Time</th></tr>"
    for row in results:
